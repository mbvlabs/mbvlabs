...
func patchSignalsRaw(c *echo.Context, signalsContents []byte, opts ...PatchSignalsOption) error {
	...
	rc := http.NewResponseController(c.Response())
	if err := rc.Flush(); err != nil {
		return fmt.Errorf("hypermedia: flush signal patch headers: %w", err)
	}
	...
	if r.Method != "GET" {
		...
		if _, err := buf.ReadFrom(r.Body); err != nil {
			if err == http.ErrBodyReadAfterClose {
				return fmt.Errorf(
					"hypermedia: read signals body after close: %w",
					err,
				)
			}
...
func Send(c *echo.Context, eventType EventType, dataLines []string, opts ...SSEEventOption) error {
	...
	rc := http.NewResponseController(c.Response())
	if err := rc.Flush(); err != nil {
		return fmt.Errorf("hypermedia: flush response: %w", err)
	}
...
func buildSSEEvent(
	...
	if err := errors.Join(
		...
		return nil, fmt.Errorf("hypermedia: encode sse event type: %w", err)
	}
	...
	if evt.EventID != "" {
		if err := errors.Join(
			...
			bytebufferpool.Put(buf)
			return nil, fmt.Errorf("hypermedia: encode sse event id: %w", err)
		}
	}
	...
	if evt.RetryDuration.Milliseconds() > 0 &&
		...
		if err := errors.Join(
			...
			bytebufferpool.Put(buf)
			return nil, fmt.Errorf("hypermedia: encode sse retry: %w", err)
		}
	}
	...
	for _, d := range evt.Data {
		if err := errors.Join(
			...
			bytebufferpool.Put(buf)
			return nil, fmt.Errorf("hypermedia: encode sse data: %w", err)
		}
	}

	if err := writeJustError(buf, doubleNewLineBuf); err != nil {
		bytebufferpool.Put(buf)
		return nil, fmt.Errorf("hypermedia: encode sse terminator: %w", err)
	}
