...
// Code generated by andurel v1.5.0; DO NOT EDIT.
package inertia
...
func Init(opts ...Option) error {
	lockFile, err := os.ReadFile("andurel.lock")
	if err != nil {
		return fmt.Errorf("inertia: read andurel.lock: %w", err)
	}
	var lock struct {
		ScaffoldConfig struct {
			InertiaRoot string `json:"inertiaRoot"`
		} `json:"scaffoldConfig"`
	}
	if err := json.Unmarshal(lockFile, &lock); err != nil {
		return fmt.Errorf("inertia: decode andurel.lock: %w", err)
	}
	rootPath := strings.TrimSpace(lock.ScaffoldConfig.InertiaRoot)
	if rootPath == "" {
		return errors.New("inertia: scaffoldConfig.inertiaRoot is required in andurel.lock")
	}
	rootHTML, err := os.ReadFile(rootPath)
	if err != nil {
		return fmt.Errorf("inertia: read root template %q: %w", rootPath, err)
	}
