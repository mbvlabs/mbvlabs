...
function submit() {
  form.post('/admin/project-inquiries')
}
</script>

<template>
  <Head title="New Project Inquiry" />

  <div class="mx-auto w-full min-w-0 max-w-7xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">New Project Inquiry</h1>
        <p class="text-muted-foreground">Add a new project inquiry.</p>
      </div>
    ...
    <form @submit.prevent="submit" class="border bg-card">
      ...
      <div class="flex justify-end gap-2 border-t px-6 py-4">
        ...
        <Button type="submit" :disabled="form.processing">
          {{ form.processing ? 'Creating...' : 'Create Project Inquiry' }}
        </Button>
      </div>
    </form>
  </div>
