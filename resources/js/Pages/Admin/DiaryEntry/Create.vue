...
const form = useForm({
  entryDate: props.item?.EntryDate ?? props.today,
  morningThoughts: props.item?.MorningThoughts ?? '',
  eveningThoughts: props.item?.EveningThoughts ?? '',
})

function submit() {
  form.post(routes.adminDiaryEntryCreate())
}
</script>

<template>
  <Head title="New Diary Entry" />

  <div class="mx-auto w-full min-w-0 max-w-7xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">New Diary Entry</h1>
        <p class="text-muted-foreground">Capture morning and evening notes for one day.</p>
      </div>
    ...
    <form @submit.prevent="submit" class="border bg-card">
      ...
      <div class="flex justify-end gap-2 border-t px-6 py-4">
        ...
        <Button type="submit" :disabled="form.processing">
          {{ form.processing ? 'Creating...' : 'Create Diary Entry' }}
        </Button>
      </div>
    </form>
  </div>
